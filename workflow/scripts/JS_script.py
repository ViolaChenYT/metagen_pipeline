...
def compare_iso_meta():
    ...
    colnames = ['SB_iso', 'DP_iso', 'AF_iso', 'SB_meta',
                'DP_meta', 'AF_meta', 'DP_bam', 'AF_bam']
    print(df)
    print(df.groupby('kind')[colnames].std())


def parse_instrain(filename, pos):
    df = pd.read_csv(filename, sep="\t", header=0)
    # print(df["scaffold"] == "NC_011750.1")
    df = df[df["scaffold"] == "NC_011750.1"]
    df["DP_meta"] = df["position_coverage"]
    df["contig"] = df["scaffold"]
...
def compare_instrain_iso():
    ...
    instrain = snakemake.input.instrain
    instrain = parse_instrain(instrain, positions)
